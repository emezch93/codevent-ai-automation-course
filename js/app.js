...
const STORAGE_KEY = "codevent_aiauto_progress_v1";
const LAST_KEY = "codevent_aiauto_last_v1";

/* ---------- Course access authorization ---------- */
const ACCESS_TOKEN_KEY = "codevent_ai_automation_access_token";
const WORKER_URL = "https://codevent-ai-automation-course.emezch93.workers.dev";
  ...
  return `
    <div class="auth-gate">
      <div class="auth-gate-card">
        ...
        <p class="auth-gate-message">Enter your access token to unlock CodeVent AI Automation.</p>
...
function renderSidebar() {
  ...
  const modulesHtml = COURSE.modules
    .map((mod, idx) => {
      ...
      const isOpen = items.some((i) => i.id === active) || localStorage.getItem("open_aiauto_" + mod.id) === "1";
...
function attachChecklistHandlers(root) {
  root.querySelectorAll("[data-checklist]").forEach((box) => {
    const key = "chk_aiauto_" + box.dataset.checklist;
...
function attachSidebarHandlers(root) {
  root.querySelectorAll("[data-toggle]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      localStorage.setItem("open_aiauto_" + modId, isOpen ? "1" : "0");
    });
  });
